...
def oploceni(sirka, vyska, cena_za_metr):
    return obvod(sirka, vyska) * cena_za_metr

print(obsah(sirka_1, vyska_1))              # 15
print(obvod(sirka_2, vyska_2))              # 60
print(oploceni(sirka_3, vyska_3, 250))       # 1000
```

Funguje to. Ale s každým dalším obdélníkem přibývají proměnné (`sirka_4`, `vyska_4`, …) a šance na záměnu — pošleš
do `obsah()` šířku jednoho obdélníka a výšku druhého a máš hloupou chybu. A pokud bys k obdélníku chtěl přidat další 
...
class Obdelnik:
    ...
    def oploceni(self, cena_za_metr):
        return self.obvod() * cena_za_metr
```

Co z ní plyne v praxi? Data (`sirka`, `vyska`) jsou uložená přímo v objektu a metody k nim přistupují přes `self`.
...
print(o1.obsah())           # 15
print(o2.obvod())           # 60
print(o3.oploceni(250))      # 1000
```

Všimni si dvou věcí:

- `obsah()` a `obvod()` nemají žádné argumenty (kromě `self`) — všechno potřebné si objekt nese s sebou.
- `oploceni()` naopak argument má — **`cena_za_metr`**. Metody můžou přijímat vstup úplně stejně jako normální funkce. Navíc uvnitř volá `self.obvod()`, takže využívá jinou metodu stejné třídy.
...
Zápis `maly.obsah()` je zkratka za `Obdelnik.obsah(maly)`.
