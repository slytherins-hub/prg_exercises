...
> **⚠️ Stav: květen 2026.** Ceny i limity se mění opravdu rychle (řádově každé 2–3 měsíce – v dubnu/květnu 2026 proběhla změna i u Codexu i u Copilotu). Bere to jako orientaci a před nasazením si vždy ověř na oficiální stránce.

### Rychlé porovnání

| Nástroj | Free tier | Placený plán s CLI | Jak se účtuje |
|---------|-----------|--------------------|---------------|
| **Claude Code** | ❌ Není zdarma (Free plán nepokrývá Code) – jen pár testovacích API kreditů na novém účtu | **Pro $20/mo** (limit ~10–40 promptů / 5 h), **Max 5× $100**, **Max 20× $200** | Buď subscription s rolling limity (5h okno), nebo per-token API |
| **Gemini CLI** | ✅ **1 000 req/den, 60 req/min** se Google účtem (od 12/2025 sníženo) | Gemini Code Assist Standard / Enterprise | Na free tieru hlavně **Flash** model; Pro je velmi omezený |
| **Codex CLI** | ⚠️ Dočasně zdarma s ChatGPT Free / Go (časově omezená promo) | **ChatGPT Plus $20/mo** (Codex zahrnut), **Pro $200** (5–20× Plus) | Kreslí z ChatGPT plánu na základě tokenů (od 2.4.2026 token-based, ne per-message) |
| **GitHub Copilot CLI** | ✅ **Zdarma pro studenty** přes GitHub Education (verifikace přes `.edu` mail nebo doklad) | **Copilot Pro $10/mo**, **Pro+ $39/mo** | Od 1.6.2026 přechod na **AI credits** (1 credit = $0.01). Pro = $10 ≈ $10 v creditech |

### Praktické tipy, jak nepřepálit

- **Sleduj limity přímo v CLI.** Většina nástrojů má příkaz pro stav: Codex `/status`, Claude Code `/cost`, Copilot CLI ukazuje limit přímo v UI.
- **Free tier obvykle běží na slabším modelu.** Gemini CLI free = hlavně Flash (rychlý, levnější), Pro je extrémně omezený. Pokud chceš špičkový model na náročný kód, počítej s placeným plánem.
- **Krátký kontext = nižší náklady.** Token-based účtování znamená, že **každý token v promptu** stojí (input i cached). Velký `CLAUDE.md` / `AGENTS.md`, hromada MCP serverů, dlouhá historie – to všechno se sčítá. Pokud Codexu/Copilotu rychle docházejí kredity, podívej se nejdřív na velikost kontextu.
- **Pro studenty:** GitHub Copilot zdarma přes [GitHub Education](https://education.github.com/) je nejjednodušší cesta (jen registrace, žádný plán neplatíš). Pro Gemini stačí Google účet.
- **Pro VUT studenty:** Některé nástroje jsou dostupné přes e-INFRA (viz [Cíl 5 – Instalace](06_cil_5_instalace_ai.md)).

> **💡 Doporučení pro tohle cvičení:** Začni s **Gemini CLI** (free tier 1 000 req/den stačí na všechny úkoly v Cíli 6) nebo **GitHub Copilot CLI** (zdarma jako student). Placené plány Claude Code / Codex Plus má smysl řešit, až budeš agenta používat denně na vlastní projekty.

---

## Společná pravidla, ať používáš cokoli

- **Před YOLO módem commitni.** `git status` čistý, `git commit -m "snapshot"` a teprve pak pust agenta na volno. Když se něco rozbije, `git checkout .` vrátí všechno zpátky.
- **Sleduj kontextové okno.** Když přesáhne ~100k tokenů, kvalita klesá. Často je rychlejší **začít novou konverzaci** (clear/new) než pokračovat v komprimovaném kontextu (compact/compress). Viz [Cíl 4 – Kontext](05_cil_4_dulezite_koncepty.md#44-kontext-co-agent-vidi-a-proc-na-tom-zalezi).
- **`/help` je tvůj kamarád.** Každý nástroj ho má, vždy ti vypíše aktuální seznam příkazů – který je pravděpodobně novější než tato stránka.
