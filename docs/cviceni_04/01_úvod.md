# CVIČENÍ 4: VÝJIMKY, CYKLY WHILE, MNOŽINY A SLOVNÍKY

Algoritmizace a programování

## ÚVOD

### Co se v tomto cvičení naučíte?

1. **Výjimky a `try/except`:**
    * Co je výjimka (`exception`)
    * Nejčastější typy výjimek (`ValueError`, `TypeError`, `KeyError`)
    * Jak bezpečně používat `try/except` bez skrytí chyb

2. **While cyklus:**
    * Syntax a fungování while cyklu
    * Rozdíl mezi `for` a `while` – kdy použít co
    * Nekonečné cykly
    * `break` a `continue` pro řízení toku cyklu
    * Validace uživatelských vstupů pomocí while

3. **Množiny (`set`):**
    * Co je množina a proč se chová jako matematická množina
    * Unikátnost prvků a odstranění duplicit
    * Hashování a rychlé ověření členství (`in`)
    * Základní operace: sjednocení, průnik, rozdíl

4. **Slovníky (`dict`):**
    * Proč slovník místo seznamu? – dict vs list
    * Vytváření slovníků (prázdné, s hodnotami)
    * Přístup k hodnotám (klíče vs indexy, kontrola existence klíče)
    * Metody slovníků (`.keys()`, `.values()`, `.items()`, `.pop()`, `.update()`)
    * Iterace přes slovníky pomocí for cyklů
    * Vnořené slovníky

5. **Procvičování:**
    * Hádání tajného čísla
    * Zpracování datového streamu
    * Translace mRNA na aminokyselinovou sekvenci
  
---

