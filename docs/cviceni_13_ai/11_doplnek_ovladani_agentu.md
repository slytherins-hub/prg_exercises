...
=== "Antigravity CLI (Google)"
    ...
    > **🔄 Nahradilo Gemini CLI:** Antigravity CLI je nástupce Gemini CLI — closed-source binárka v Go, ovládá se příkazem **`agy`** (ne `gemini`).
    ...
    Spustí interaktivní rozhraní (TUI). Stejně jako Claude Code dokáže číst a upravovat soubory, spouštět příkazy, atd. Pro jednorázový (neinteraktivní) dotaz ze skriptu:
    ...
    ```bash
    agy --dangerously-skip-permissions     # přeskočí všechna potvrzování
    ```

    Za běhu se dá režim potvrzování přepínat příkazem `/permissions` — nabízí mj. *request-review* (default), *proceed-in-sandbox* a *always-proceed* (bez ptaní). Pro neinteraktivní běh (CI) použij `agy --headless --approve all`.

    > **⚠️ Pozor:** Stejně jako u Claude Code – používej **jen v sandboxu**. (Antigravity má i vlastní OS-level sandbox: na Linuxu nsjail, na macOS `sandbox-exec`.)

    ### Návrat do předchozí konverzace

    Konverzace se ukládají **automaticky**. Při ukončení `agy` vypíše příkaz, kterým konkrétní session obnovíš. Uvnitř běžícího CLI navíc funguje:

    ```
    /resume                       # interaktivní browser předchozích sessionů
    ```
    ...
    | Příkaz | Co dělá |
    |--------|---------|
    | `/model` | Přepne použitý model (Gemini Flash/Pro, podle plánu i Claude / GPT-OSS) |
    | `/diff` | Ukáže rozdíly v souborech, které agent změnil |
    | `/rewind` / `/undo` | Vrátí konverzaci o krok zpět |
    | `/permissions` | Změní režim potvrzování (request-review / sandbox / always-proceed) |
    | `/resume` | Obnoví předchozí session |
    | `/mcp` | Spravuje připojené MCP servery |
    | `/agents` | Spravuje subagenty |
    | `?` | Nápověda – seznam příkazů |
    | `/exit` nebo `/quit` | Ukončí session |

    > **💡 Tip:** Konvence projektu zadáš souborem `AGENTS.md` v kořeni projektu (Gemini CLI dříve používal `GEMINI.md`).
...
| **Antigravity CLI** | ✅ `@./img.png` (nebo `@./slozka/`) | ✅ | ❌ zatím ne |
