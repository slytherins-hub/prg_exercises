...
> **🖥️ Slide:** Celý přehled nástrojů si můžeš otevřít jako [samostatný slide](09_slide_prehled.md){target="_blank"}.
...
| **Antigravity CLI** (Google, nástupce Gemini CLI) | **zdarma** s Google účtem; Google AI Pro/Ultra na vyšší limity | velké refaktoringy díky **1M kontextu**, free volba pro studenty; **Flash model odpovídá v sekundách** (rychlejší než minuty u silných modelů), ale s nižší kvalitou |
...
> **🧠 Důležitý koncept — agent ≠ model:** Claude Code, Codex i Copilot jsou **lokálně běžící programy** (CLI/plugin), které ve smyčce *„zavolej model → přečti odpověď → spusť nástroj → zopakuj"* obsluhují AI model. **Agent je smyčka a nástroje, model je „mozek"** — jsou to dvě oddělené vrstvy. Proto můžeš stejného agenta napárovat na **libovolný kompatibilní model**: Claude Code defaultně volá Anthropic, ale stejně dobře umí volat e-INFRA (Qwen/DeepSeek/GPT-OSS), Amazon Bedrock nebo třeba lokální Ollama. Tvůrci defaultně dávají vlastní modely (logické — vydělávají na nich), ale technicky tě to nesvazuje. Kombinace „cizí agent + jiný model" je v 2026 zcela běžná — proto se např. v GitHub Copilotu CLI dá přepnout `/model` na Claude, GPT i Gemini. Práce s různými agenty je přitom velmi podobná – nástroje se liší v detailech, ale základní princip je u všech stejný.
