# Cvičení 13 (jen AI): AI nástroje pro programování

> **ℹ️ Poznámka:** Toto je výběr z [Cvičení 13](../cviceni_13/README.md) — jsou tu jen části věnované **AI nástrojům** (bez OOP a dědičnosti). Číslování cílů je zachované z původního cvičení.

## Obsah
1. [ÚVOD](01_uvod.md)
2. [CÍL 3: PŘEHLED AI NÁSTROJŮ](04_cil_3_prehled_ai_nastroju.md)
3. [CÍL 4: DŮLEŽITÉ KONCEPTY AI NÁSTROJŮ](05_cil_4_dulezite_koncepty.md)
4. [CÍL 5: INSTALACE AI NÁSTROJŮ](06_cil_5_instalace_ai.md)
5. [CÍL 6: UKÁZKY – AI V PRAXI](07_cil_6_ukazky.md)
6. [CÍL 7: BEZPEČNOST A PRAVIDLA POUŽÍVÁNÍ AI](08_cil_7_bezpecnost_a_pravidla.md)
7. [Slide – přehled AI nástrojů](09_slide_prehled.md)
8. [Doplněk – aktuální AI modely (duben 2026)](10_doplnek_modely.md)
9. [Doplněk – ovládání AI agentů](11_doplnek_ovladani_agentu.md)
10. [SELF-CHECK: PROCVIČENÍ ZNALOSTÍ](12_self_check_procvičení_znalostí.md)
11. [DOPORUČENÉ ZDROJE](13_doporucene_zdroje.md)
12. [NEJDŮLEŽITĚJŠÍ RADY](14_nejdulezitejsi_rady.md)
13. [DOPLNĚK: UŽITEČNÉ MCP SERVERY A SKILLY](15_doplnek_mcp_servery.md)

---

## Otevři si tuhle stránku v mobilu

<p style="text-align:center;">
  <img src="qr_stranka.svg" alt="QR kód na tuto stránku" style="width:260px;max-width:80%;height:auto;">
</p>

<p style="text-align:center;font-size:1.6rem;font-weight:700;word-break:break-all;">
  <a href="https://slytherins-hub.github.io/prg_exercises/cviceni_13_ai/">https://slytherins-hub.github.io/prg_exercises/cviceni_13_ai/</a>
</p>
