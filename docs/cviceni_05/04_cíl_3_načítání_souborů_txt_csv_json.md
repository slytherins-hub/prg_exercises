...
## CÍL 3: NAČÍTÁNÍ A ZAPISOVÁNÍ SOUBORŮ (TXT, CSV, JSON)

Při reálném použití program obvykle nepracuje jen s daty v kódu.  
Častý scénář je:
...
Soubory si dej do složky `data` ke svému skriptu.

---

### 3.1 Proč je potřeba soubor otevřít (a proč `with open(...)`)?
...
V zápisu `with open("...", mode="režim") as file:` je `file` objekt souboru, přes který pak voláš `read()`, `write()` atd.
...
### 3.2 `read()`, `readline()`, `readlines()` - kdy co použít
